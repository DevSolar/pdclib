...
/* C99 specifies that C++ implementations should define these macros only if
   ...
   specified that no such definition should be necessary, and C11 no longer
   makes that requirement either. PDCLib goes with the C11/C++11 specification.
*/
...
/* C99 specifies that C++ implementations should define these macros only if
   __STDC_CONSTANT_MACROS is defined before <stdint.h> is incuded. C++11, however,
   specified that no such definition should be necessary, and C11 no longer
   makes that requirement either. PDCLib goes with the C11/C++11 specification.
*/
...
#define RSIZE_MAX ( _PDCLIB_SIZE_MAX / 2 )
#endif
