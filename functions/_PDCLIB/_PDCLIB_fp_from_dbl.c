...
int main( void )
{
    ...
    TESTCASE( fp.scale == ( 1 | ( _PDCLIB_DBL_MANT_DIG - 1 ) << 8 ) );
    ...
    TESTCASE( _PDCLIB_bigint_cmp( &fp.mantissa, &t ) == 0 );
    TESTCASE( fp.scale == ( 1<<1 | ( _PDCLIB_DBL_MANT_DIG - 1 ) << 8 ) );
    TESTCASE( fp.exponent == (_PDCLIB_bigint_digit_t)(1 - ( _PDCLIB_DBL_MAX_EXP - 1 ) ) );
#endif
